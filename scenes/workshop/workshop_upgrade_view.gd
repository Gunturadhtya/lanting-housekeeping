class_name WorkshopUpgradeView
extends Control
...
func show_cards(cards : Array[CardResource]) -> void:
	...
	for card in sorted_cards:
		...
		if card.upgraded:
			...
			overlay.tooltip_text = "Already Upgraded"
		else:
			overlay.tooltip_text = "Upgrade this card"
			overlay.pressed.connect(_on_card_pressed.bind(card))
		wrapper.add_child(overlay)

	title_label.text = "Upgrade a Card"
	show()
